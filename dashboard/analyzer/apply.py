...
async def apply_curated_result(
    ...
    # Use the timestamp of the newest claimed segment for decisions/key moments
    newest_seg_ts = max((s.get("ts") or s.get("created_at") for s in claimed_segments), default=None)
                    ...
                    if m.get("id"):
                        ...
                        if existing:
                            ...
                            await conn.execute(
                                ...
                                VALUES (%s, %s, %s, %s, %s, %s, %s, 'ai', COALESCE(%s, now()), %s)
                                """,
                                (
                                    recording_uuid,
            ...
            for dec in result.get("decisions") or []:
                try:
                    if dec.get("id"):
                        ...
                        await conn.execute(
                            "INSERT INTO decision (recording_id, description, status, segment_id, decided_at, topic_id, dedup_hash) VALUES (%s, %s, %s, %s, COALESCE(%s, now()), %s, %s)",
                            (recording_uuid, desc, dec.get("status", "concept"), claimed_segments[-1]["id"], newest_seg_ts, topic["id"] if topic else None, dedup),
                        )
                except Exception as exc:
            ...
            for gu in result.get("goal_updates") or []:
                try:
                    ...
                    await conn.execute(
                        ...
                        WHERE id = %s
                        """,
                        (new_status, gu.get("coaching_tip"), new_status, new_status, claimed_segments[-1]["id"], goal["id"]),
                    )
