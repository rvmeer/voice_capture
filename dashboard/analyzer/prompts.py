...
- agenda_mode=dynamic: ALWAYS maintain at least one active agenda item. If no items exist yet, create one for the current topic. Add a new item whenever the conversation topic clearly shifts; mark the previous one done. Never leave the agenda empty.
- To CREATE a new item: omit 'id', include 'title' (and optionally status/topic_ref).
- To UPDATE an existing item: include its 'id' from the current state. Never invent IDs.
...
EXAMPLE_OUTPUT = {
    ...
    "goal_updates": [],
    "new_goals": [],
    "agenda": {"active_item_id": None, "items": [{"title": "Datamigratieplan", "status": "active", "topic_ref": "datavoorbereiding"}]},
    "add_synonyms": [],
}
